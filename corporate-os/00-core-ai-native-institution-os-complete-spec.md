...
**Definition**
制度にとっての現実をどの ontology で表現し、どの state を正本とし、どの memory を保持し、どの reusable skill を executable capability unit として管理し、どの read/write surface を canonical とするかを定義する次元。
...
- reusable skill package の ownership、 version、 approval state を明示する。
...
5. memory と workflow のあいだに、 skill taxonomy、 skill relation graph、 skill package library を維持する。
...
3. relevant skill を discovery / activation し、必要な場合は compose して deliberation に組み込む。
4. 重要な tradeoff がある場合は複数 option を生成する。
5. uncertainty、 novelty、 agency impact を明示的に評価する。
...
5. reusable skill / playbook の activation 前に、 safety、 completeness、 executability、 maintainability、 cost-awareness の評価ゲートを通す。
...
**Definition**
OSが平時において、自らの policy、 threshold、 model、 skill、 soft rule を、 hard constraint を破らずに更新する方法を定義する次元。
...
5. reusable skill は `create -> evaluate -> analyze -> consolidate -> publish -> deprecate` の lifecycle で管理する。
...
**Definition**
Defines the official ontology, state model, memory types, reusable skills as executable capability units, provenance rules, and canonical read/write surfaces.
...
- Ownership, version, and approval state are explicit for reusable skill packages.
...
5. Maintain a skill taxonomy, skill relation graph, and skill package library between memory and workflow.
...
3. Discover and activate relevant skills, and compose them where necessary before material deliberation.
4. Generate multiple options where material tradeoffs exist.
5. Explicitly score uncertainty, novelty, and agency impact.
...
- Effective skill utilization rate in deliberation
...
5. Evaluate reusable skills and playbooks on safety, completeness, executability, maintainability, and cost-awareness before activation.
...
**Definition**
Defines how the OS updates its policies, thresholds, models, skills, and soft rules in steady state without violating hard constraints.
...
5. Govern reusable skills through a lifecycle of create, evaluate, analyze, consolidate, publish, and deprecate.
